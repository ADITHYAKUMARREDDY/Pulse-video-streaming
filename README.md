...
2. Testing video upload functionality
3. Testing video streaming
4. Testing role-based access control
5. Testing multi-tenant isolation

## Assumptions and Design Decisions

1. **Sensitivity Analysis**: Currently simulated. In production, integrate with AI/ML models or content moderation APIs (e.g., Google Vision API, AWS Rekognition).

2. **Video Processing**: Currently copies files to processed directory. In production, implement actual video processing with FFmpeg for optimization and transcoding.

3. **File Storage**: Uses local file system. In production, consider cloud storage (AWS S3, Google Cloud Storage) for scalability.

4. **Tenant ID**: Auto-generated on registration. In production, implement proper tenant management system.

5. **Video Streaming**: Uses HTTP range requests. For large-scale deployment, consider CDN integration.

6. **Authentication**: Token passed in URL for video streaming (ReactPlayer limitation). In production, consider implementing signed URLs or proxy server.
...
Developed as a full-stack assignment project.
